...
$(document).ready(function () {
    ...
    $slideshow.bind("turning", function (event, page, view) {
        if (page === 1) {
            ...
            if (page === Object.keys(services).length * 2 + 2) {
    ...
    $('.resize').click(function () {
        if ($('.brochure-container').hasClass('container')) {
            ...
            $('.introduction .brochure-col').css('left', '-50vw');
            console.log($slideshow);
        } else {
            $('.introduction .brochure-col').css('left', 'auto');
            resizeSmall();
        }
    });
...
function resizeSmall() {
    ...
    $slideshow.turn('size', 860, 485);
    $(".introduction .col-6").css("width", "calc(100% / 2)");
    $slideshow.css("left", "-430px");
}
...
function setThumbnailState() {
    $(".thumbnail-container").each(function() {
        ...
        var $choseField = $(this).children(".check");
        if(localStorage[service]) {
            $choseField.text('Chosen');
            $choseField.removeClass("edge-btn-white");
            $choseField.addClass("edge-btn-pink");
        } else {
            $choseField.text('Choose');
            $choseField.removeClass("edge-btn-pink");
            $choseField.addClass("edge-btn-white");
        }
    });
}
